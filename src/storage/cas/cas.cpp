...
CAS::CAS(mongocxx::collection collection) : collection_(std::move(collection)) {}

std::string CAS::compute_hash(const bsoncxx::document::view& geometry,
...
bool CAS::store(const std::string& hash, const bsoncxx::document::view& geometry,
                const bsoncxx::document::view& attributes) {
    try {
        if (exists(hash)) {
            return true;
        ...
        doc << "hash" << hash << "geometry"
            << bsoncxx::types::b_document{bsoncxx::document::value(geometry)} << "attributes"
            << bsoncxx::types::b_document{bsoncxx::document::value(attributes)} << "created_at"
...
std::string CAS::serialize_for_hashing(const bsoncxx::document::view& geometry,
                                       const bsoncxx::document::view& attributes) {
...
std::vector<std::unique_ptr<BPO>> CAS::find_in_bbox(double min_lon, double min_lat, double max_lon,
                                                    double max_lat) {
    std::vector<std::unique_ptr<BPO>> results;
